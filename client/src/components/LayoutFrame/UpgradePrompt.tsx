			...
			{props.balances.map(balance => (
				<ListItem key={balance.version}>
					<Grid container
						...
						alignItems="stretch"
					>
						<Grid item key="versionItem">
							<h3>Version: {balance.version}</h3>
						</Grid>
						<Grid item key="incubating">
							<Grid container direction="row" justify="flex-start" alignItems="center" spacing={3}>
								<Grid item>
									<h4>Incubating: {API.fromWei(balance.incubating)}</h4>
								</Grid>
								<Grid item>
									<h4>Actual: {API.fromWei(balance.actual)}</h4>
								</Grid>
								<Grid item>
									<Button color="secondary" variant="contained" onClick={async () => await API.Contracts.VersionController.claimAndRedeem(balance.version).send({ from: props.walletAddress })}>Claim and Redeem</Button>
								</Grid>
							</Grid>
						</Grid>
					</Grid>
