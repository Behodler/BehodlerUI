...
export default function Prometheus(props: prometheusProps) {
    ...
    const executeSeed = async (kharon: string, scarcity: string, weidai: string, dai: string, registry: string) => walletContextProps.contracts.behodler.Prometheus.seed(kharon, scarcity, weidai, dai, registry).send(primaryOptions)
    const executeStealFlame = async (token: string, kharonToll: string, buyer: string) => walletContextProps.contracts.behodler.Prometheus.stealFlame(token, kharonToll, buyer).call(primaryOptions)
...
function Seed(props: seedProps) {
    ...
    return <Paper>
        ...
        <List>
            ...
            <ListItem>
                <Button variant="contained" color="secondary" onClick={async () => await props.execute(kharon, scarcity, weidai, dai, registry)}>Execute</Button>
            </ListItem>
        </List>
    </Paper>
}
...
function StealFlame(props: stealFlameProps) {
    ...
    return <Paper>
        ...
        <List>
            ...
            <ListItem key="button">
                <Button variant="contained" color="secondary" onClick={async () => { const out = await props.execute(token, kharonToll, buyer); setOutput(out.toString()) }}>Execute</Button>
            </ListItem>
            <ListItem key="out">
                {output}
            </ListItem>
