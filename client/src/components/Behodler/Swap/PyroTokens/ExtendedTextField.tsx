...
export default function ExtendedTextField(props: props) {
  ...
  const allowanceEffects = useEth ? API.generateNewEffects(props.address, walletContextProps.account, useEth, props.decimalPlaces) : currentTokenEffects
  useEffect(() => {
    const effect = allowanceEffects.allowance(walletContextProps.account, props.addressToEnableFor || walletContextProps.contracts.behodler.Behodler.address)
