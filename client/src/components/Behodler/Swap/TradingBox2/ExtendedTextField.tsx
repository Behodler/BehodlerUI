...
function ExtendedTextField(props: props) {
    ...
    const indexOfAddressFunc = (address: string) =>
        props.dropDownFields.findIndex((t) => t.address.toLowerCase().trim() == address.toLowerCase().trim());
    ...
    const currentTokenEffects = API.generateNewEffects(
        props.address,
    ...
    useEffect(() => {
        const effect = currentTokenEffects.allowance(
            walletContextProps.account,
        ...
        const subscription = effect.Observable.subscribe((allowance) => {
            ...
            const en =
                (props.scarcityAddress !== undefined &&
                    props.address.trim().toLowerCase() === props.scarcityAddress.trim().toLowerCase()) ||
    ...
    useEffect(() => {
        if (props.exchangeRate && props.exchangeRate.setReserve) {
            const effect = currentTokenEffects.balanceOfTokenEffect(
                walletContextProps.contracts.behodler.Behodler2.Behodler2.address
            );
            const subscription = effect.Observable.subscribe((balance) => {
                                    ...
                                    onClick={async () =>
                                        await API.enableToken(
                                            ...
                                            props.addressToEnableFor ||
                                                walletContextProps.contracts.behodler.Behodler.address
                                        )
                                    }>
